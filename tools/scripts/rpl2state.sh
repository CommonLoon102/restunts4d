...
# This script generates the .BIN and .BNI files out of .rpl files.
# The filenames must be in 0000_0.rpl format, where the number
# after the underscore is the remainder of the first number when
# divided by 8. This way this script can be run in parallel by
# starting it with different partition parameter.
# Errors are logged into a text file, so desyncing replays can be
# identified and copied later.

partition="$1"

GAME_DIR="$HOME/games/stunts/repldump"
CONFIG="$GAME_DIR/dosbox.proc.conf"
output_file="$GAME_DIR/partition_$partition.txt"

files=("$GAME_DIR"/*_"$partition".rpl)
