...
static legacy_u16 reference_sar16(legacy_u16 bits, legacy_u16 count)
{
	...
	while (count-- != 0U) {
		bits = (legacy_u16)((bits >> 1) | (bits & LEGACY_U16_SIGN_BIT));
	}
	return bits;
}
...
static void test_dword_shifts_and_rotates(void)
{
	...
	for (index = 0U; index < sizeof(values) / sizeof(values[0]); index++) {
		for (count = 0U; count < TEST_SHIFT_COUNT_LIMIT; count++) {
			...
			assert(LEGACY_U32_ROL(LEGACY_U32_ROR(values[index], count), count) == values[index]);
		}
	}
}
...
static void test_multiply_and_divide(void)
{
	...
	assert(LEGACY_U16_REPLACE_LOW_BYTE(TEST_WORD_PATTERN, TEST_REPLACEMENT_WORD) ==
		   TEST_WORD_WITH_REPLACEMENT_WORD_LOW_BYTE);
	...
	assert(LEGACY_U8_WRAP_MUL(LEGACY_U8_MAX, 2U) == TEST_U8_DOUBLE_MAX_RESULT);
	assert(LEGACY_S16_WRAP_ADD(32767, 1) == -32768);
	assert(LEGACY_S16_WRAP_SUB(-32768, 1) == 32767);
	assert(LEGACY_S32_WRAP_ADD(LEGACY_S32_FROM_BITS(LEGACY_S32_MAX), 1L) ==
		   LEGACY_S32_FROM_BITS(LEGACY_U32_SIGN_BIT));
	assert(LEGACY_S32_WRAP_ADD_S16(LEGACY_S32_FROM_BITS(LEGACY_S32_MAX), 1) ==
		   LEGACY_S32_FROM_BITS(LEGACY_U32_SIGN_BIT));
	assert(LEGACY_S32_WRAP_SUB_S16(LEGACY_S32_FROM_BITS(LEGACY_U32_SIGN_BIT), 1) ==
	...
	assert(LEGACY_U16_DIV_OR_ZERO(LEGACY_U16_MAX, 2U) == LEGACY_S16_MAX);
	...
	assert(LEGACY_U32_DIV_OR_ZERO(LEGACY_U32_MAX, 2UL) == LEGACY_S32_MAX);
	...
	assert(LEGACY_S32_DIV_OR_ZERO(LEGACY_S32_FROM_BITS(LEGACY_U32_SIGN_BIT), -1L) == 0L);
}
...
static void test_little_endian_access(void)
{
	...
	assert(bytes[0] == TEST_DWORD_BYTE_ZERO && bytes[1] == TEST_DWORD_BYTE_ONE);
	assert(bytes[2] == TEST_DWORD_BYTE_TWO && bytes[3] == TEST_DWORD_BYTE_THREE);
	assert(LEGACY_READ_U32_LE(bytes) == TEST_DWORD_PATTERN);
	assert((legacy_u32)LEGACY_READ_S32_LE(bytes) == TEST_DWORD_PATTERN);
	assert(LEGACY_U32_FROM_WORDS(TEST_PATTERN_LOW_WORD, TEST_PATTERN_HIGH_WORD) ==
		   TEST_DWORD_PATTERN);
	assert(LEGACY_U32_FROM_WORDS(-1, LEGACY_S16_FROM_BITS(LEGACY_U16_SIGN_BIT)) ==
