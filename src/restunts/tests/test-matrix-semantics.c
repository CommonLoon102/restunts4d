...
static void test_matrix_products(void)
{
	...
	for (left_index = 0U; left_index < sizeof(values) / sizeof(values[0]); left_index++) {
		for (right_index = 0U; right_index < sizeof(values) / sizeof(values[0]); right_index++) {
			clear_matrix(&matrix);
			...
			expected = reference_matrix_product(values[left_index], values[right_index]);
