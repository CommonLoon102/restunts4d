...
void run_opponent_menu(void)
{
	...
	for (;;) {
		if (displayed_opponent != (legacy_u8)gameconfig.game_opponenttype) {
			...
			displayed_opponent = (legacy_u8)gameconfig.game_opponenttype;
			...
			if ((legacy_u8)gameconfig.game_opponenttype != OPPONENT_NONE) {
				description = locate_text_res(opponent_resource, opponent_description_id);
			} else {
				description =
					locate_text_res((legacy_s8 far *)miscptr, opponent_racing_car_label_id);
			}
				...
				if (character == ']') {
					if (line_length != 0) {
						*(&resID_byte1 + line_length) = 0;
						font_draw_text(
							&resID_byte1, OPPONENT_DESCRIPTION_X,
		...
		elapsed = (legacy_u16)menu_animate_button_highlight(selected, opponentmenu_buttons,
		...
		if (key == KEY_LEFT) {
			selected = selected == OPPONENT_MENU_PREVIOUS_BUTTON ? OPPONENT_MENU_DONE_BUTTON
		...
		if (key == KEY_RIGHT) {
			selected = selected < OPPONENT_MENU_DONE_BUTTON ? (legacy_u8)(selected + 1U)
		...
		if (selected == OPPONENT_MENU_PREVIOUS_BUTTON) {
			gameconfig.game_opponenttype =
				(legacy_s8)((legacy_u8)gameconfig.game_opponenttype - 1U);
