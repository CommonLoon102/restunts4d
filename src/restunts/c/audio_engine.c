										 ...
										 void far *source_pointer)
{
	...
	for (field = 0; field < AUDIO_ENGINE_DEFINITION_SIZE; field++) {
		source = (const legacy_u8 far *)dos_memory_make_pointer(source_segment, source_offset);
							 ...
							 void far *shape_resources, void far *audio_resources)
{
	...
	rate = LEGACY_U16_DIV_OR_ZERO(engine_definition->sample_count, divisor);
	rate =
								 ...
								 legacy_s16 second_y, legacy_s16 second_z, legacy_s16 interval)
{
	...
	timer = &audio_timers[index];
	second_distance = (legacy_u16)polarRadius2D(polarRadius2D(second_x, second_z), second_y);
	if (LEGACY_S16_FROM_BITS(second_distance) > AUDIO_MAX_AUDIBLE_DISTANCE) {
		timer->target_volume = 0;
		return;
	}

	first_distance = (legacy_u16)polarRadius2D(polarRadius2D(first_x, first_z), first_y);
	distance_delta = LEGACY_U16_WRAP_SUB(first_distance, second_distance);
	quotient = LEGACY_U16_DIV_OR_ZERO(AUDIO_SPATIAL_INTERVAL_PERCENT, (legacy_u16)interval);
	...
	definition = (const legacy_u8 far *)audio_read_far_pointer(
		(legacy_u8 *)&engine_definition->resources[0]);
	divisor = definition[AUDIO_ENGINE_RATE_DIVISOR_OFFSET];
	base_rate = LEGACY_U16_DIV_OR_ZERO((legacy_u16)base_value, divisor);
	base_rate = LEGACY_U16_WRAP_ADD(
		base_rate, (legacy_u16)((legacy_u16)definition[AUDIO_ENGINE_RATE_BASE_OFFSET]
								<< AUDIO_ENGINE_RATE_BASE_SHIFT));
	denominator = LEGACY_U16_WRAP_SUB(AUDIO_MAX_AUDIBLE_DISTANCE, scaled_delta);
	if (denominator != 0) {
...
void audio_enable_effects(void)
{
	...
	for (channel = AUDIO_EFFECT_CHANNEL_FIRST; channel <= AUDIO_EFFECT_CHANNEL_LAST; channel++) {
...
void audio_release_channel_range(legacy_s16 first_channel, legacy_s16 last_channel)
{
	...
	if (dos_audio_uses_direct_channels == 0) {
		...
		while (channel <= last_channel) {
			...
			context = dos_audio_contexts;
			for (context_index = 0; context_index < AUDIO_CONTEXT_COUNT; context_index++) {
...
static legacy_s16 audio_find_driver_context(legacy_u8 far *resource, struct AUDIO_CHANNEL *timer)
{
	...
	resource_mask = resource_read_u16le(resource + AUDIO_RESOURCE_CONTEXT_MASK_OFFSET);
	if (resource_mask == 0) {
		return -1;
	}
							...
							legacy_u8 note, legacy_u16 parameter, legacy_s16 handle)
{
	...
	resource = (legacy_u8 far *)audio_read_far_pointer((legacy_u8 *)&timer->resource);
	...
	driver_channel = dos_audio_uses_direct_channels == 0 ? context_index : timer->driver_channel;
...
static legacy_s16 audio_find_free_sfx_channel(void)
{
	...
	for (candidate = AUDIO_EFFECT_CHANNEL_FIRST; candidate <= AUDIO_EFFECT_CHANNEL_LAST;
		 candidate++) {
									 ...
									 legacy_u16 rate)
{
	...
	if (audio_effect_rate != 0) {
		scaled_rate = (legacy_u16)((legacy_u32)(legacy_u16)rate * AUDIO_EFFECT_RATE_SCALE);
	...
	resource_data_offset = (legacy_u16)bytes[AUDIO_EFFECT_RESOURCE_ENTRY_COUNT_OFFSET] *
											 ...
											 legacy_u8 priority)
{
	...
	rate = timer->current_volume >> AUDIO_TIMER_INTERPOLATION_FRACTION_BITS;
	resource = audio_read_far_pointer((legacy_u8 *)&timer->definition.resources[resource_index]);
...
void audio_fade_out(legacy_s16 delay_ticks)
{
	...
	volume = uses_direct_channels != 0 ? AUDIO_DEFAULT_MASTER_VOLUME : audio_music_rate;
	while (volume > 0) {
		audio_update_lock = AUDIO_UPDATE_LOCKED;
