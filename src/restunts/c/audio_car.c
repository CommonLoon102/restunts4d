...
static void audio_carstate_offsets(struct VECTOR far *previous, struct VECTOR far *current,
								   const struct VECTOR *camera_previous,
								   const struct VECTOR *camera_current,
								   const struct VECTOR *car_previous,
											 ...
											 legacy_u8 flags)
{
	...
	if ((desired & CAR_SOUND_SKID_MASK) != 0) {
		if ((flags & CAR_SOUND_SKID_MASK) == (desired & CAR_SOUND_SKID_MASK)) {
			return flags;
		}
...
void audio_carstate(void)
{
	...
	if (is_in_replay != 0) {
		if (audio_car_state_ready != 0) {
			...
			if (((legacy_u8)audio_player_car_flags & CAR_SOUND_SKID_MASK) != 0) {
				audio_stop_skid_sound(audio_player_engine_channel);
			}
			if (((legacy_u8)audio_player_car_flags & CAR_SOUND_ENGINE_ACTIVE_FLAG) != 0) {
				audio_stop_engine(audio_player_engine_channel);
			}
			if (gameconfig.game_opponenttype != 0) {
				if (((legacy_u8)audio_opponent_car_flags & CAR_SOUND_SKID_MASK) != 0) {
					audio_stop_skid_sound(audio_opponent_engine_channel);
				}
				if (((legacy_u8)audio_opponent_car_flags & CAR_SOUND_ENGINE_ACTIVE_FLAG) != 0) {
	...
	player_previous.x =
		audio_carstate_position((legacy_s32)state.playerstate.car_previous_position.lx);
	player_previous.y =
		audio_carstate_position((legacy_s32)state.playerstate.car_previous_position.ly);
	player_previous.z =
		audio_carstate_position((legacy_s32)state.playerstate.car_previous_position.lz);
	player_current.x = audio_carstate_position((legacy_s32)state.playerstate.car_position.lx);
	player_current.y = audio_carstate_position((legacy_s32)state.playerstate.car_position.ly);
	player_current.z = audio_carstate_position((legacy_s32)state.playerstate.car_position.lz);

	if (gameconfig.game_opponenttype != 0) {
		opponent_previous.x =
			audio_carstate_position((legacy_s32)state.opponentstate.car_previous_position.lx);
		opponent_previous.y =
			audio_carstate_position((legacy_s32)state.opponentstate.car_previous_position.ly);
		opponent_previous.z =
			audio_carstate_position((legacy_s32)state.opponentstate.car_previous_position.lz);
		opponent_current.x =
			audio_carstate_position((legacy_s32)state.opponentstate.car_position.lx);
		opponent_current.y =
			audio_carstate_position((legacy_s32)state.opponentstate.car_position.ly);
		opponent_current.z =
			audio_carstate_position((legacy_s32)state.opponentstate.car_position.lz);
	}

	if (cameramode == CAMERA_MODE_FOLLOW) {
		camera_current = state.game_follow_camera_position[(legacy_u8)followOpponentFlag];
		camera_previous = followOpponentFlag != 0 ? state.game_opponent_camera_previous
	...
	audio_carstate_offsets(&record->player_previous, &record->player_current, &camera_previous,
						   &camera_current, &player_previous, &player_current);
	record->player_rpm = state.playerstate.car_currpm;

	car_count = 1;
	if (gameconfig.game_opponenttype != 0) {
		audio_carstate_offsets(&record->opponent_previous, &record->opponent_current,
							   &camera_previous, &camera_current, &opponent_previous,
							   &opponent_current);
