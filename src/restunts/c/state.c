...
void update_player_tick(legacy_s8 input_flags) {
	...
	if (state.playerstate.car_crashBmpFlag != CRASH_EVENT_NONE) {
		...
		if (state.playerstate.car_actual_speed == CAR_SPEED_STOPPED) {
			...
			if (state.playerstate.car_rev_speed == CAR_SPEED_STOPPED && state.playerstate.car_wheel_vertical_speed[0] == 0 && state.playerstate.car_wheel_vertical_speed[1] == 0 && state.playerstate.car_wheel_vertical_speed[2] == 0 && state.playerstate.car_wheel_vertical_speed[3] == 0) {
				return ;
			}
		}
	}

	update_car_speed(input_flags, PLAYER_CAR_INDEX,
	...
	update_player_steering_input(
		LEGACY_S16_SAR((legacy_s16)input_flags,
	...
	state.game_travDist = LEGACY_S32_WRAP_ADD(
		state.game_travDist,
		(legacy_s32)(legacy_u16)state.playerstate.car_actual_speed);
	...
	if (route_distance_or_direction != 0) {
		...
		if (skipped_route_count == PENALTY_ROUTE_OUTSIDE_TRACK) {
			...
			if (state.game_player_route_status == ROUTE_TRACKING_NORMAL) {
				...
				} else if (skipped_route_count ==
					PENALTY_ROUTE_FINISH_REACHED ||
					skipped_route_count > PENALTY_ROUTE_DECISION_DISTANCE) {
					if (td01_track_file_cpy[state.game_player_previous_route] == route_index ||
						td02_penalty_related[state.game_player_previous_route] == route_index) {
						state.game_route_confirmation_count = LEGACY_S8_WRAP_ADD(
							state.game_route_confirmation_count, ROUTE_CONFIRMATION_STEP);
					} else {
						if (td01_track_file_cpy[route_index] == state.game_player_previous_route ||
							td02_penalty_related[route_index] == state.game_player_previous_route) {
		...
		if (commit_penalty != 0) {
			...
			if (skipped_route_count > 0) {
				penalty_time = LEGACY_S16_WRAP_MUL(
					LEGACY_S16_WRAP_MUL(
						skipped_route_count, framespersec),
	...
	state.game_player_route_indicator = ROUTE_INDICATOR_NONE;
	if (state.game_player_route_status != ROUTE_TRACKING_OUTSIDE_TRACK) {
		world_to_car_rotation = mat_rot_zxy(
			...
			if (state.playerstate.car_route_index != ROUTE_INDEX_NONE) {
				if ((previous_route_status == ROUTE_TRACKING_NORMAL ||
					...
					td02_penalty_related[state.game_player_confirmed_route] ==
						state.playerstate.car_route_index)) {
					route_delta_or_best_depth.x = LEGACY_S16_WRAP_SUB(
					...
					route_delta_or_best_depth.z = LEGACY_S16_WRAP_SUB(
						state.playerstate.car_route_target.z,
						position_to_word(
							state.playerstate.car_position.lz));
					mat_mul_vector(&route_delta_or_best_depth, world_to_car_rotation, &local_route_delta);
		...
		if (route_selection_required != 0) {
			if (td02_penalty_related[route_index] != TRACK_ROUTE_LINK_NONE) {
				guidance_required = 0;
			} else {
				route_end_reached = 0;
				...
				do {
					...
					route_point_delta(&world_route_delta,
				...
				if (state.game_player_route_status == ROUTE_TRACKING_WRONG_WAY) {
					if (selected_route_point == ROUTE_POINT_FIRST) {
						...
						get_track_route_point(route_index, route_segment_start,
							(legacy_s16)LEGACY_S8_WRAP_SUB(
								selected_route_point, ROUTE_POINT_STEP), 0);
						get_track_route_point(route_index, route_segment_end,
							(legacy_s16)(legacy_u8)selected_route_point, 0);
					}
					route_distance_or_direction = LEGACY_S16_FROM_BITS((legacy_u16)LEGACY_S16_WRAP_SUB(
						state.playerstate.car_rotate.x,
						polarAngle(
							LEGACY_S16_WRAP_SUB(
								route_segment_start[0].x, route_segment_end[0].x),
							LEGACY_S16_WRAP_SUB(
								route_segment_end[0].z, route_segment_start[0].z))) & ANGLE_MASK);
		...
		if (guidance_required != 0 &&
			...
			state.playerstate.car_route_heading_error = LEGACY_S16_FROM_BITS(
				(legacy_u16)polarAngle(
					LEGACY_S16_WRAP_NEGATE(local_route_delta.x), local_route_delta.z) & ANGLE_MASK);
			if (state.playerstate.car_crashBmpFlag == CRASH_EVENT_NONE) {
				route_distance_or_direction = LEGACY_U16_SAR(LEGACY_U16_WRAP_ADD(
		...
		if (state.playerstate.car_lap_count != 0) {
			route_distance_or_direction = multiply_and_scale(cos_fast(track_angle),
				LEGACY_S16_WRAP_SUB(trackcenterpos[startrow2],
					position_to_word(
						state.playerstate.car_position.lz)));
			route_distance_or_direction = LEGACY_S16_WRAP_ADD(route_distance_or_direction,
