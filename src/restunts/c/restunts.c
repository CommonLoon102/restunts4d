...
static void far* audio_snare_resource;
static void far* audio_tom_resource;
static void far* audio_ride_resource;
static void far* audio_crash_resource;
static void far* audio_closed_hihat_resource;
static void far* audio_open_hihat_resource;
	...
	void far* shape_resources, void far* audio_resources)
{
	...
	rate = LEGACY_U16_DIV_OR_ZERO(
		LEGACY_READ_U16_LE(context), divisor);
...
legacy_s16 audio_start_sample(legacy_u16 value, legacy_s16 handle)
{
	...
	if (LEGACY_READ_U16_LE(context + 0x10U) != dos_memory_pointer_offset(resource) ||
		LEGACY_READ_U16_LE(context + 0x12U) != dos_memory_pointer_segment(resource)) {
	...
	context[0x28U] = 0;
	context[0x29U] = 0;
	LEGACY_WRITE_U16_LE(context + 0x2AU, dos_memory_pointer_offset(timer));
	...
	for (context_index = 0; context_index < dos_audio_context_count;
		...
		if (context[0x16U] == 4U) {
			...
			if (level <= 0) {
				...
				chunk[0x15U]--;
				dos_audio_driver_end_context(context[0x2CU], context);
				audio_channel_notes[context[0]] = 0;
			}
		}
