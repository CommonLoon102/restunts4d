...
static unsigned short dos_alloc_upper(unsigned short paras) {
	...
	/* DOS allocation strategy 40h means upper-memory-only, first fit. */
	if (dos_set_umb_link_state(1) && dos_set_alloc_strategy(0x40)) {
...
#ifdef RESTUNTS_DOS
/*
 * Large VSH resources briefly need both their source and worst-case parsed
 * copy. The VGA A000-AFFF aperture is addressable in real mode, so allow it
 * to be used as scratch space until the parsed block has been compacted.
 */
unsigned short mmgr_borrow_video_memory(void) {
	unsigned short oldlimit;

	oldlimit = resendptr2->resofs;
	if (oldlimit < 0xB000) {
...
void mmgr_restore_video_memory(unsigned short oldlimit) {
	...
	usedend = resptr2->resofs + resptr2->ressize;
	if (usedend > oldlimit) {
...
int mmgr_reserve_upper_memory(unsigned short paras) {
	/* One temporary reservation is enough for the race framebuffer. */
